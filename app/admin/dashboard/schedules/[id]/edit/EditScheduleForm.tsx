...
import { updateScheduleAction } from '@/lib/actions/schedules';
...
export default function EditScheduleForm({ schedule }: { schedule: any }) {
  ...
  const [isFull, setIsFull] = useState(schedule.seatsAvailable?.trim().toUpperCase() === 'FULL');
  const [seats, setSeats] = useState(schedule.seatsAvailable?.trim().toUpperCase() === 'FULL' ? '' : schedule.seatsAvailable);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await updateScheduleAction(schedule.id, formData);
      if (result?.error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input 
              ...
              type="text" 
              required 
              defaultValue={schedule.monthYear}
          ...
          <div>
            ...
            <input 
              ...
              type="text" 
              required 
              defaultValue={schedule.packageName}
          ...
          <div>
            ...
            <select 
              name="hotel1Stars"
              defaultValue={schedule.hotel1Stars}
          ...
          <div>
            ...
            <select 
              name="hotel2Stars"
              defaultValue={schedule.hotel2Stars}
              className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#B48421] focus:border-transparent outline-none transition-all bg-white"
            >
              <option value="3">3 Bintang</option>
              <option value="4">4 Bintang</option>
          ...
          <div>
            ...
            <input 
              name="airlineLogo"
              type="text" 
              defaultValue={schedule.airlineLogo}
